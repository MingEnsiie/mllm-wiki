...
- **资料指南** — 每个资料的专属面板，含 LLM 生成摘要、关键主题和建议问题
- **URL / YouTube / 粘贴文本来源** — 支持导入网页、YouTube 字幕和直接粘贴文本
- **有范围限制的聊天** — 将查询范围限定到指定资料，并附 AI 生成的后续问题建议
- **音频概述** — NotebookLM 风格的双主持人播客脚本生成，支持 TTS 合成与内置播放器
...
原始设计中没有。每个资料现在有专属的**资料指南面板**，打开时即可快速了解内容：

- **LLM 生成摘要** —— 简明概述资料涵盖的内容
- **关键主题** —— 提取的核心主题和要素
- **建议问题** —— 基于资料内容生成的可直接发问的问题列表
- 首次打开时生成并缓存，支持一键重新生成

### 19. URL / YouTube / 粘贴文本来源

除本地文件导入外，现在支持多种来源类型：

- **URL 导入** —— 通过内置阅读器抓取并摄入任意网页
- **YouTube 导入** —— 提取 YouTube 视频字幕并作为资料摄入
- **粘贴文本** —— 直接粘贴原始文本，立即进入摄入流程
- 三种类型均经过相同的两步思维链摄入管线处理

### 20. 有范围限制的聊天

原始设计中没有。聊天面板现在支持**按资料范围限定查询**：

- **范围选择器** —— 将查询范围限定到一个或多个特定资料，而非整个 Wiki
- **有依据的回答** —— LLM 回答锚定所选资料，减少幻觉
- **后续问题建议** —— 每次回答后 AI 自动生成上下文相关的追问建议
- 范围选择在同一对话的多条消息中持续有效

### 21. 音频概述（NotebookLM 风格）

原始设计中没有。将你的知识库转化为**双主持人播客**：

- **脚本生成** —— LLM 编写涵盖所选资料的自然双主持人对话，可配置范围和风格
- **TTS 合成** —— 通过 OpenAI TTS 或兼容端点，为主持人 A 和主持人 B 分别合成不同声音
- **内置音频播放器** —— 内嵌播放器，支持播放/暂停、进度条、时间显示及逐行发言人高亮
- **持久化脚本** —— 生成的脚本保存到 `wiki/studio/audio/<id>/`，可随时重播
- **TTS 设置** —— 在设置中配置提供商、API 密钥、自定义端点、模型、声音 A/B 和语速
...
3. 进入 **资料源** → 导入文档（PDF、DOCX、MD 等）、URL、YouTube 链接或粘贴文本
4. 观察 **活动面板** —— LLM 自动构建 Wiki 页面
5. 打开资料的**资料指南**，即刻获取摘要、关键主题和建议问题
6. 使用 **聊天** 查询你的知识库（可选择将范围限定到特定资料）
7. 点击资料源面板中的 🎧 按钮，生成知识库的**音频概述**播客
8. 浏览 **知识图谱** 查看关联
9. 查看 **审核** 处理需要你关注的项目
10. 定期运行 **Lint** 维护 Wiki 健康度
